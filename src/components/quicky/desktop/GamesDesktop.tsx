...
// Quicky — DESKTOP GAMES HUB (Game Hub PRD §67 + Web Premium §39-§43)
// DB-driven catalog: featured game hero (with REAL live numbers from the
// dashboard endpoint), the full game grid from /api/quicky/games, and the
// league progression block. No fabricated events, no fake player counts.
...
export function GamesDesktop() {
  ...
  const play = () => {
    ...
    if (featured) openGameLanding(featured.slug)
    else openGameLanding('spin-the-bottle')
  }

  return (
    <div className="flex flex-col gap-10">
      {/* Page purpose: title + one primary action */}
      ...
      <section data-testid="games-featured">
        {gamesLoaded && !featured ? (
          <div className="rounded-3xl border border-white/8 bg-[var(--qk-card)]/50 p-10 flex flex-col items-center gap-2 text-center">
            <span className="text-4xl" aria-hidden>🎮</span>
            <p className="font-bold">More games are coming soon.</p>
            <p className="text-sm text-white/50">Check back soon for new ways to play.</p>
          </div>
        ) : (
          <button
            onClick={play}
            className={`qk-featured group relative w-full text-left rounded-3xl overflow-hidden border border-white/10 ${featured ? artworkGradient(featured.artwork) : 'bg-gradient-to-br from-[var(--qk-accent)] via-[#B23A6E] to-[var(--qk-purple)]'}`}
          ...
          >
            ...
            <div className="relative flex items-center gap-8 p-8 min-[1600px]:p-10">
              <div className="shrink-0 w-24 h-24 min-[1600px]:w-28 min-[1600px]:h-28 rounded-3xl bg-black/25 border border-white/15 flex items-center justify-center text-6xl transition-transform duration-200 group-hover:scale-105" aria-hidden>
                {featured?.icon ?? '🍾'}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-[11px] font-bold tracking-[0.22em] text-white/70 uppercase">Featured game</p>
                <h2 className="text-3xl font-black text-white mt-1.5 tracking-tight">{featured?.name ?? 'Spin the Bottle'}</h2>
                <p className="text-sm text-white/85 mt-2 max-w-[52ch] leading-relaxed">
                  {featured?.shortDescription ?? 'Join a live table with up to 12 players — spins, dares, kisses and gifts in real time.'}
                </p>
              ...
              <div className="shrink-0 flex flex-col items-center gap-2">
                <span className="qk-cta flex items-center gap-2 rounded-full bg-white text-[var(--qk-accent)] font-black text-sm px-7 py-3.5 shadow-xl">
                  ...
                  {roomId ? 'RETURN TO TABLE' : featured?.isPlayable ? 'PLAY NOW' : 'VIEW GAME'}
                </span>
