...
export function EmailScreen({ inline = false }: { inline?: boolean }) {
        ...
        {step === 'edit' ? (
          ...
          <>
            <div className="bg-[var(--qk-accent)]/10 border border-[var(--qk-accent)]/30 rounded-2xl p-3 text-center">
              ...
              <p className="text-sm font-semibold text-[var(--qk-accent-light)] mt-0.5">{email}</p>
            </div>
            <div className={inline ? 'flex items-stretch gap-2' : 'block'}>
              <input
                type="text"
              ...
              {inline && (
                <button
                  ...
                  disabled={loading}
                  className="shrink-0 bg-coral-gradient glow-coral rounded-2xl px-5 font-semibold text-sm flex items-center justify-center gap-2 disabled:opacity-50 active:scale-[0.98] transition-transform"
                  data-testid="email-verify-inline"
                >
