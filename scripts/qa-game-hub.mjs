...
// ── Ludo room REVISED: MODE tables (2P/4P) + diagonal seat fill + join-race hardening ──
ok('ludo join REVISED: MODE tables (2P duel / 4P table) + DIAGONAL seat fill 1→3→2→4 + join lock', has('src/lib/quicky/ludo-assignment.ts', 'LUDO_SEAT_FILL_ORDER: readonly number[] = [0, 2, 1, 3]', 'normalizeLudoMode', 'maxPlayers: mode', "status: { in: ['WAITING', 'STARTING'] }", "throw new Error('room_locked')") && has('src/app/api/quicky/games/ludo/join/route.ts', 'normalizeLudoMode(body?.mode)', 'assignLudoRoomAndSeat(me.id, mode)', 'playersCount >= claim.maxPlayers'))
ok('ludo join RACE HARDENING (mobile P2028/P2003): room+first seat in ONE transaction, tx timeout/maxWait, P2028 retry, P2003 absorbed', has('src/lib/quicky/ludo-assignment.ts', 'TX_OPTS = { timeout: 15_000, maxWait: 10_000 }', 'tx.spinRoom.create', 'tx.spinRoomPlayer.create', "e?.code === 'P2028'", "e?.code === 'P2003'", 'sleep(400)', 'attempt < 2'))
ok('ludo mode picker BEFORE starting: 2 Players / 4 Players segmented control above Play Now (persisted)', has('src/components/quicky/game-primary/GamePrimaryScreen.tsx', 'playExtra?: React.ReactNode', 'playExtra}') && has('src/components/quicky/ludo/LudoLanding.tsx', 'ludo-mode-pick', 'ludo-mode-${m}', '{m} Players', 'aria-checked={mode === m}', 'quicky-ludo-mode', 'api.ludo.join(mode)'))
ok('ludo waiting REVISED: web overlays the board; MOBILE uses the round-bar strip; mode-aware counts; chat stays open', has('src/components/quicky/ludo/LudoGameArea.tsx', "roomStatus === 'WAITING'", '{snapshot.players.length}/{snapshot.maxPlayers} joined') && has('src/components/quicky/ludo/LudoRoundBar.tsx', 'playersJoined', 'maxPlayers', 'Waiting for players — ${playersJoined}/${maxPlayers} joined · chat is open') && src('src/components/quicky/ludo/ludo-room.css').includes('display: none; /* mobile default — round bar owns the waiting state */'))
ok('ludo CLIENT PREDICTION: tap moves the coin INSTANTLY via the shared engine; server validates async and always wins on mismatch', has('src/store/ludo-room.ts', 'CLIENT-SIDE PREDICTION', 'engineMoveToken(game, meId, tokenId', 'forceNextSnapshot', 'ctl.forceNextSnapshot = true'))
ok('ludo ROUND-4 DICE SEQUENCER: one phase machine (enter→roll→settle→hold→exit) replaced the boolean pile — no more appear/vanish dice', has('src/components/quicky/ludo/useDiceSequencer.ts', 'export function useDiceSequencer', "'hidden'", "'entering'", "'rolling'", "'settling'", "'revealed'", "'exiting'", 'lastSeenRef', 'freeAtRef') && has('src/components/quicky/ludo/LudoGameArea.tsx', 'useDiceSequencer(game?.lastRoll', 'diceSettled && legalIds.has(t.id)', 'dice.freeAtRef.current - Date.now()'))
ok('ludo ROUND-4 ROLL RECORD: lastRoll survives every roll path — no-move and six-cancelled throws are VISIBLE on every client (the silent turn-skip is dead)', has('src/lib/quicky/ludo/types.ts', 'LudoRollRecord', 'lastRoll: LudoRollRecord | null', 'turnId: string | null') && has('src/lib/quicky/ludo/rules.ts', 'state.lastRoll = { rollId: makeRollId(now, rng), playerId, value, rolledAt: now }', 'state.turnId = `turn_${state.turnNumber}_'))
ok('ludo ROUND-4 NEVER FROZEN: crypto dice + always-on tick keepalive heals a stalled auto-roll/watchdog chain within seconds', has('src/lib/quicky/ludo-server.ts', "import { randomInt } from 'node:crypto'", 'const secureRng =', 'randomInt(0, 2 ** 31) / 2 ** 31') && has('src/app/api/quicky/games/ludo/tick/route.ts', 'ensureLudoRuntime', 'POST') && has('src/store/ludo-room.ts', 'api.ludo.tick(roomId)', '7_000') && has('src/lib/quicky/api-client.ts', 'tick: (roomId: string)'))
ok('ludo ROUND-4 SETTLE-THEN-MOVE: the die fades out on a phase-driven exit BEFORE any coin hops; coins selectable only after the value settles', has('src/components/quicky/ludo/LudoDice.tsx', 'phase', 'FACE_ORIENTATION', 'ROLL_X', "ease: 'easeOut'", "type: 'spring'") && has('src/components/quicky/ludo/ludo-room.css', 'ldo-settled', 'preserve-3d', '@keyframes ldo-dice-tumble', 'perspective'))
